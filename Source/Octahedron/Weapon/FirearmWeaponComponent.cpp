...
#include "FirearmWeaponComponent.h"
...
UFirearmWeaponComponent::UFirearmWeaponComponent()
{
	BoundsScale = 2.f;
...
void UFirearmWeaponComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UFirearmWeaponComponent::PressedFire()
{
	...
	if (Character->GetFPAnimInstance()->Montage_IsPlaying(FPReloadAnimation))
	{
		Character->GetFPAnimInstance()->Montage_Stop(0.f, FPReloadAnimation);
	}
	...
	case EFireMode::Single:
		//Character->GetWorldTimerManager().SetTimer(FireRateDelayTimerHandle, this, &UFirearmWeaponComponent::SingleFire, FireDelay, false, 0.f);
		SingleFire();
		Character->GetWorldTimerManager().SetTimer(FireRateDelayTimerHandle, FireDelay, false);
		break;
	case EFireMode::Burst:
		Character->GetWorldTimerManager().SetTimer(FireRateDelayTimerHandle, this, &UFirearmWeaponComponent::BurstFire, FireDelay, true, 0.f);
		break;
	case EFireMode::Auto:
		Character->GetWorldTimerManager().SetTimer(FireRateDelayTimerHandle, this, &UFirearmWeaponComponent::FullAutoFire, FireDelay, true, 0.f);
		break;
	default:
		break;
	}
...
void UFirearmWeaponComponent::ReleasedFire()
{
	IsPlayerHoldingShootButton = false;
}

void UFirearmWeaponComponent::PressedReload()
{
...
void UFirearmWeaponComponent::PressedSwitchFireMode()
{
...
void UFirearmWeaponComponent::SwitchFireMode()
{
	switch (FireMode)
	{
	case EFireMode::Single:
...
void UFirearmWeaponComponent::SingleFire()
{
	Fire();
...
void UFirearmWeaponComponent::BurstFire()
{
	Fire();
	BurstFireCurrent++;
...
void UFirearmWeaponComponent::FullAutoFire()
{
	Fire();
...
void UFirearmWeaponComponent::Fire()
{
	...
	if (IsProjectileWeapon)
	{
		...
		if (World != nullptr)
		{
			...
			NewOnWeaponProjectileFireDelegate.Broadcast(MuzzleTraceResult); // projectile is spawned in bp
		}
	}
	else // hitscan weapon
	{
		...
		CurrentMagazineCount = FMath::Max(CurrentMagazineCount - 1, 0);
		NewOnWeaponHitScanFireDelegate.Broadcast(MuzzleTraceResults);
	}

	NewOnFireDelegate.ExecuteIfBound();
	...
	{
		PlayAnimation(FPFireAnimation, false);
	}

	Character->MakeNoise(1.f, Character, GetComponentLocation());
}

void UFirearmWeaponComponent::StopFire()
{
	RecoilStop();
}

void UFirearmWeaponComponent::ForceStopFire()
{
	StopFire();
}

void UFirearmWeaponComponent::Stow()
{
	Character->SetCurrentWeapon(nullptr);

	NewOnWeaponChangeDelegate.BindUFunction(Cast<UFPAnimInstance>(Character->GetMesh1P()->GetAnimInstance()), FName("StowCurrentWeapon"));
	NewOnWeaponChangeDelegate.Execute(this);
	...
	Character->GetWorldTimerManager().SetTimer(EquipDelayTimerHandle, this, &UFirearmWeaponComponent::SetIsStowingFalse, EquipTime, false);
}

void UFirearmWeaponComponent::Equip()
{
	NewOnWeaponChangeDelegate.BindUFunction(Cast<UFPAnimInstance>(Character->GetMesh1P()->GetAnimInstance()), FName("SetCurrentWeapon"));
	NewOnWeaponChangeDelegate.Execute(this);

	NewOnFireDelegate.BindUFunction(Cast<UFPAnimInstance>(Character->GetMesh1P()->GetAnimInstance()), FName("Fire"));
	...
	if (Character != nullptr && FPReloadAnimation != nullptr)
	{
		if (Character->GetFPAnimInstance())
		{
			Character->GetFPAnimInstance()->Montage_Play(FPEquipAnimation, 1.f);

			FOnMontageBlendingOutStarted BlendOutDelegate;
			BlendOutDelegate.BindUObject(this, &UFirearmWeaponComponent::EquipAnimationBlendOut);
			Character->GetFPAnimInstance()->Montage_SetBlendingOutDelegate(BlendOutDelegate, FPEquipAnimation);
		}
	}
}

void UFirearmWeaponComponent::EquipAnimationBlendOut(UAnimMontage* animMontage, bool bInterrupted)
{
	SetIsEquippingFalse();
}

void UFirearmWeaponComponent::SetIsEquippingFalse()
{
	// Ensure the timer is cleared by using the timer handle
...
void UFirearmWeaponComponent::SetIsStowingFalse()
{
	...
	IsStowing = false;
	NewOnStowDelegate.Broadcast(Character, this);
}

void UFirearmWeaponComponent::OnReloaded()
{
...
void UFirearmWeaponComponent::Reload()
{
	...
	if (Character != nullptr && FPReloadAnimation != nullptr)
	{
		if (Character->GetFPAnimInstance())
		{
			...
			Character->GetFPAnimInstance()->Montage_Play(FPReloadAnimation, 1.f);

			// remove this blend out logic and just call OnReloaded from the anim notify whatever
			/*FOnMontageBlendingOutStarted BlendOutDelegate;
			BlendOutDelegate.BindUObject(this, &UFirearmWeaponComponent::ReloadAnimationBlendOut);
			Character->GetFPAnimInstance()->Montage_SetBlendingOutDelegate(BlendOutDelegate, ReloadAnimation);*/
		}
	}
}

void UFirearmWeaponComponent::CancelReload(float BlendTime)
{
	SetIsReloadingFalse();
	if (Character != nullptr && FPReloadAnimation != nullptr)
	{
		...
		{
			AnimInstance->Montage_Stop(BlendTime, FPReloadAnimation);
...
void UFirearmWeaponComponent::PressedADS()
{
	ADS_Held = true;
...
void UFirearmWeaponComponent::EnterADS()
{
	if (IsReloading || IsEquipping || IsStowing)
	{
		return;
...
void UFirearmWeaponComponent::ReleasedADS()
{
	ADS_Held = false;
	ADSTL->Reverse();
}

void UFirearmWeaponComponent::ExitADS(bool IsFast)
{
	if (IsFast)
	{
...
void UFirearmWeaponComponent::ADSTLCallback(float val)
{
...
//void UFirearmWeaponComponent::AttachWeapon(AOctahedronCharacter* TargetCharacter)
//{
...
//		{
//			// Fire
//			EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Started, this, &UFirearmWeaponComponent::PressedFire);
//			EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Completed, this, &UFirearmWeaponComponent::ReleasedFire);
//
//			// Reload
//			EnhancedInputComponent->BindAction(ReloadAction, ETriggerEvent::Triggered, this, &UFirearmWeaponComponent::PressedReload);
//
//			// Switch Fire Mode
//			EnhancedInputComponent->BindAction(SwitchFireModeAction, ETriggerEvent::Triggered, this, &UFirearmWeaponComponent::PressedSwitchFireMode);
//
//			// ADS
//			EnhancedInputComponent->BindAction(ADSAction, ETriggerEvent::Triggered, this, &UFirearmWeaponComponent::PressedADS);
//			EnhancedInputComponent->BindAction(ADSAction, ETriggerEvent::Completed, this, &UFirearmWeaponComponent::ReleasedADS);
//		}
//
//		CanFire = true;
//	}
...
//void UFirearmWeaponComponent::DetachWeapon()
//{
...
//}
//
//bool UFirearmWeaponComponent::InstantDetachWeapon()
//{
//	// Check that the character is valid, and the currently set weapon is this object
...
void UFirearmWeaponComponent::RecoilStart()
{
	if (RecoilCurve)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(FireTimer, this, &UFirearmWeaponComponent::FireTimerFunction, RecoilToStableTime, false);

		GetWorld()->GetTimerManager().SetTimer(RecoilTimer, this, &UFirearmWeaponComponent::RecoilTimerCallback, (1.f / 60.f), true);
	}
}

void UFirearmWeaponComponent::FireTimerFunction()
{
	GetWorld()->GetTimerManager().PauseTimer(FireTimer);
}

//Called when firing stops
void UFirearmWeaponComponent::RecoilStop()
{
	IsShouldRecoil = false;
}

void UFirearmWeaponComponent::RecoilTimerCallback()
{
	float recoiltime;
	FVector RecoilVec;
...
void UFirearmWeaponComponent::RecoveryStart()
{
	if (PCRef->GetControlRotation().Pitch > RecoilStartRot.Pitch)
	{
		GetWorld()->GetTimerManager().SetTimer(StopRecoveryTimer, this, &UFirearmWeaponComponent::StopRecoveryTimerFunction, RecoveryTime, false);
		GetWorld()->GetTimerManager().SetTimer(RecoilRecoveryTimer, this, &UFirearmWeaponComponent::RecoilRecoveryTimerCallback, (1.f / 60.f), true);
	}
}

//This function too is automatically called from the recovery start function.
void UFirearmWeaponComponent::StopRecoveryTimerFunction()
{
...
void UFirearmWeaponComponent::RecoilRecoveryTimerCallback()
{
...
void UFirearmWeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UFirearmWeaponComponent::SetIsReloadingFalse()
{
	IsReloading = false;
